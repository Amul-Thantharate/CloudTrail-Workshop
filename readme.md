...
We’re then going to see how our AWS API calls appear in the log files, we’re going to check the checksum / hash of the log files, look at the log file hashes in the digest files, and then we will try modifying a log file _and_ a digest file, to see how it breaks validity.
...
Go to _Buckets_ and click on <kbd>Create bucket</kbd>
...
Go to _Trails_ and click <kbd>Create trail</kbd>
...
On the next page, leave all the settings as default. For this demo we only care about management events (AWS API calls), _not_ data events (S3 reads and writes for example)
...
Once you’re done, wait _one hour_ and then proceed to the next step. CloudTrail log digests are created each hour, so we’ll wait for that to complete before proceeding.
...
Click on _Buckets_ and go into your CloudTrail bucket you created earlier
...
The CloudTrail _log_ file I’m viewing is
...
First we’ll look at the hash value of the _CloudTrail log_ we were viewing earlier, in the digest file it says that log file `AWSLogs/1234567789012/CloudTrail/ap-southeast-4/2023/03/15/1234567789012_CloudTrail_ap-southeast-4_20230315T0320Z_1iwuEXInvYIpo1Zq.json.gz` has the hash `3007368a401b56dab0576c7cc9c3acb4ff41d423c7d403242d15d3f4f49f21d2`
...
Let’s see what happens if we modified _anything_ in the CloudTrail log, let’s throw Bill Gates under the bus and say _he_ called the `ListFunctions20150331` API
...
## Stage 5 - _Verifying the log and digest files_
...
So this is telling us that each log file _and_ digest file created between 5AM and 6AM UTC was valid and unmodified.
...
_Make sure you select the file ending in .gz_
...
Go to _Trails_ and select the `demo` trail you created, and click <kbd>Delete</kbd>
...
_Note:_ CloudTrail may add a few files to your bucket for a few minutes after your Trail has been deleted, so if you empty your bucket, and then go to delete it and it says there’s still objects in there, this will be why. Just wait a few minutes and try again.
...
**/ Contributing /**
[github] (https://github.com/Amul-Thantharate/CloudTrail-Workshop)
happy Codig :smile: :smile: :smile:
**/ END OF WORKSHOP /**
